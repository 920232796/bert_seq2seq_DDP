...
model_save_dir = "./state_dict/roberta_auto_title_model/" # 训练好的模型保存位置。
lr = 1e-5
maxlen=256
...
trainer = Trainer(env_type="pytorch", epoches=5, model_save_dir=model_save_dir,
                  val_every_step=500, device=device, batch_size=16)
...
def main():
    ...
    trainer.train(bert_model, optimizer, train_dataset=train_dataset, validation_func=validate,
                  val_dataset=None, compute_metric_func=None, collate_fn=bert_seq2seq_collate_fn)
